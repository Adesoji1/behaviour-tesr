...
| 3 | Retrain frequency (daily) | ⚠️ | The **pull** is auto-scheduled (`sync`, `BP_SYNC_AT_HOUR`) and the **"retrain due" §4 check + Slack alert** run automatically after each pull. The **retrain itself is MANUAL by design for now** (`ml.retrain_trigger --run`, gated). Automation is written but disabled (`deploy/crontab.example`). See "Retraining today: MANUAL, with automatic alerts". |
...
| 9 | Drift detection (gradual / sudden) | ⚠️ | PSI-based amount drift detection can trigger retraining. Gradual and sudden drift are not currently handled as separate detection paths. |
